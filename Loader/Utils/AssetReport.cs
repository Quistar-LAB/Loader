...
namespace Loader.Utils {
    internal static class AssetReport {
        ...
        private static Dictionary<string, Item> assets = new Dictionary<string, Item>(256);
        private static List<List<Package.Asset>> duplicates = new List<List<Package.Asset>>(4);
        private static readonly string[] allHeadings = { "Buildings and parks", "Props", "Trees", "Vehicles", "Citizens", "Nets", "Nets in buildings and parks",
                                                         "Props in buildings, parks and nets", "Trees in buildings, parks and nets" };
        private static readonly CustomAssetMetaData.Type[] allTypes = { CustomAssetMetaData.Type.Building, Prop, CustomAssetMetaData.Type.Tree, CustomAssetMetaData.Type.Vehicle,
                                                                        CustomAssetMetaData.Type.Citizen, Road, Road, Prop, CustomAssetMetaData.Type.Tree };
        private static int texturesShared, materialsShared, meshesShared;

        private static string filepath;
        private static StreamWriter w;
        private static readonly char[] forbidden = { ':', '*', '?', '<', '>', '|', '#', '%', '&', '{', '}', '$', '!', '@', '+', '`', '=', '\\', '/', '"', '\'' };
        private const string steamid = @"<a target=""_blank"" href=""https://steamcommunity.com/sharedfiles/filedetails/?id=";
        private const string privateAsset = @"<a target=""_blank"" href=""http://steamcommunity.com/workshop/filedetails/discussion/667342976/357284767251931800/"">Asset bug";
        private const string spaces = "&nbsp;&nbsp;";
        private const string enableOption = @", enable the option ""Load used assets"".";

        internal static void AssetFailed(Package.Asset assetRef) {
            Item item = FindItem(assetRef);
            if (item != null)
                item.Failed = true;
        }

        internal static void Duplicate(List<Package.Asset> list) => duplicates.Add(list);

        internal static void AddPackage(Package p, CustomAssetMetaData lastmeta, bool enabled, bool useddir) {
        ...
        internal static void AddPackage(Package p) {
        ...
        internal static bool IsKnown(Package.Asset assetRef) => assets.ContainsKey(assetRef.fullName);
        internal static bool IsKnown(string fullName) => assets.ContainsKey(fullName);

        internal static void AddReference(Package.Asset knownRef, string fullName, CustomAssetMetaData.Type type) {
        ...
        internal static void AddMissing(string fullName, CustomAssetMetaData.Type type) {
            if (!assets.TryGetValue(fullName, out Item child))
        ...
        private static void SetIndirectUsages() {
        ...
        private static Dictionary<Item, List<Item>> GetUsedBy() {
        ...
        internal static void Save(int textures, int materials, int meshes) {
            try {
        ...
        private static void Report(IEnumerable<Item> items, string[] headings, CustomAssetMetaData.Type[] types, params int[] usages) {
            int usage = 0;
        ...
        private static void ReportMissing(IEnumerable<Item> items, Dictionary<Item, List<Item>> usedBy, string[] headings, CustomAssetMetaData.Type[] types, params int[] usages) {
        ...
        private static void Div(string line) => w.WriteLine(string.Concat("<div>", line, "</div>"));
        private static void Div(string cl, string line) => w.WriteLine(Cl(cl, line));
        private static void Italics(string line) => Div("<i>" + line + "</i>");
        private static void H1(string line) => w.WriteLine(string.Concat("<h1>", line, "</h1>"));
        private static void H2(string line) => w.WriteLine(string.Concat("<h2>", line, "</h2>"));
        private static void H3(string line) => w.WriteLine(string.Concat("<h3>", line, "</h3>"));
        private static void Stat(string stat, object value, string unit) => Div("my", Cl("st", stat) + Cl(value.ToString() + spaces + unit));

        private static bool FromWorkshop(Item item) => FromWorkshop(item.packageName);
        private static bool FromWorkshop(string packageName) => ulong.TryParse(packageName, out ulong id) && id > 99999999;
        private static string Ref(Item item) => item.NameChanged ? Enc(item.name) : Ref(item.packageName, item.name);
        private static string Ref(string packageName, string name) => FromWorkshop(packageName) ? string.Concat(steamid, packageName, "\">", Enc(name), "</a>") : Enc(name);
        private static string Ref(string packageName) => FromWorkshop(packageName) ? string.Concat(steamid, packageName, "\">", "Workshop item ", packageName, "</a>") : Enc(packageName);
        private static string Cl(string cl, string s) => string.Concat("<div class=\"", cl, "\">", s, "</div>");
        private static string Cl(string s) => string.Concat("<div>", s, "</div>");
        private static string Name(Item item) => item.name;

        private static Item FindItem(Package.Asset assetRef) {
